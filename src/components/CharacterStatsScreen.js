...
function CharacterStatsScreen(props) {
  const { name, strength, dexterity, intelligence, charisma, gold, exp, level, maxHP, currentHP, maxMP, currentMP, maxStamina, currentStamina } = props.playerCharacter;
  return (
    <React.Fragment>
      <h4>{name}</h4>
      <h4>Level: {level}  EXP: {exp}</h4>
      <h5>Gold: {gold}g</h5>
